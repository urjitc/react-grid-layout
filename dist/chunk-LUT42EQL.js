...
function resolveCompactionCollision(layout, item, moveToCoord, axis, hasStatics) {
  ...
  for (let i = itemIndex + 1; i < layout.length; i++) {
    ...
    if (chunkNTZV4CE2_js.collides(item, otherItem)) {
  ...
  if (!allowOverlap) {
    if (compactV) {
      if (typeof b === "number") {
        ...
        l.y = Math.min(chunkNTZV4CE2_js.bottom(compareWith), l.y);
      }
      while (l.y > 0 && !chunkNTZV4CE2_js.getFirstCollision(compareWith, l)) {
        l.y--;
      }
    } else if (compactH) {
      while (l.x > 0 && !chunkNTZV4CE2_js.getFirstCollision(compareWith, l)) {
        l.x--;
      }
    }
  }
  let collision;
  while ((collision = chunkNTZV4CE2_js.getFirstCollision(compareWith, l)) !== void 0 && !allowOverlap) {
    if (compactH) {
...
function compact(layout, compactType, cols, allowOverlap) {
  ...
  let b = chunkNTZV4CE2_js.bottom(compareWith);
  const sorted = chunkNTZV4CE2_js.sortLayoutItems(layout, compactType);
  const out = new Array(layout.length);
  for (let i = 0; i < sorted.length; i++) {
    const sortedItem = sorted[i];
    if (sortedItem === void 0) continue;
    let l = chunkNTZV4CE2_js.cloneLayoutItem(sortedItem);
    if (!l.static) {
...
function compactItem(compareWith, l, compactType, cols, fullLayout, allowOverlap, maxY) {
  return compactItemInternal(
    compareWith,
    chunkNTZV4CE2_js.cloneLayoutItem(l),
    compactType,
    cols,
    fullLayout,
    allowOverlap,
...
function resolveCompactionCollision2(layout, item, moveToCoord, axis, hasStatics) {
  ...
  item[axis] += 1;
  const itemIndex = layout.findIndex((l) => l.i === item.i);
  const layoutHasStatics = hasStatics ?? chunkNTZV4CE2_js.getStatics(layout).length > 0;
  for (let i = itemIndex + 1; i < layout.length; i++) {
    const otherItem = layout[i];
    if (otherItem === void 0) continue;
    if (otherItem.static) continue;
...
function compactItemVertical(compareWith, l, fullLayout, maxY) {
  ...
  let collision;
  while ((collision = chunkNTZV4CE2_js.getFirstCollision(compareWith, l)) !== void 0) {
...
var verticalCompactor = {
  ...
  compact(layout, _cols) {
    const compareWith = chunkNTZV4CE2_js.getStatics(layout);
    let maxY = chunkNTZV4CE2_js.bottom(compareWith);
    const sorted = chunkNTZV4CE2_js.sortLayoutItemsByRowCol(layout);
    const out = new Array(layout.length);
    for (let i = 0; i < sorted.length; i++) {
      const sortedItem = sorted[i];
      if (sortedItem === void 0) continue;
  ...
  onMove(layout, item, x, y, _cols) {
    const newLayout = chunkNTZV4CE2_js.cloneLayout(layout);
...
var horizontalCompactor = {
  ...
  compact(layout, cols) {
    const compareWith = chunkNTZV4CE2_js.getStatics(layout);
    const sorted = chunkNTZV4CE2_js.sortLayoutItemsByColRow(layout);
    const out = new Array(layout.length);
    for (let i = 0; i < sorted.length; i++) {
      const sortedItem = sorted[i];
      if (sortedItem === void 0) continue;
...
var noCompactor = {
  ...
  allowOverlap: false,
  compact(layout, _cols) {
    return chunkNTZV4CE2_js.cloneLayout(layout);
  },
  onMove(layout, item, x, y, _cols) {
    const newLayout = chunkNTZV4CE2_js.cloneLayout(layout);
    const movedItem = newLayout.find((l) => l.i === item.i);
...
function findOrGenerateResponsiveLayout(layouts, breakpoints, breakpoint, lastBreakpoint, cols, compactType) {
  ...
  if (existingLayout) {
    return chunkNTZV4CE2_js.cloneLayout(existingLayout);
  }
  ...
  const clonedLayout = chunkNTZV4CE2_js.cloneLayout(layout || []);
  return compact(chunkNTZV4CE2_js.correctBounds(clonedLayout, { cols }), compactType, cols);
}
...
//# sourceMappingURL=chunk-LUT42EQL.js.map
//# sourceMappingURL=chunk-LUT42EQL.js.map