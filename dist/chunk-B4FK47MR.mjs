...
function collides(l1, l2, threshold = 0) {
  if (l1.i === l2.i) return false;
  if (l1.x + l1.w <= l2.x + threshold) return false;
  if (l1.x >= l2.x + l2.w - threshold) return false;
  if (l1.y + l1.h <= l2.y + threshold) return false;
  if (l1.y >= l2.y + l2.h - threshold) return false;
  return true;
}
function getFirstCollision(layout, layoutItem, threshold = 0) {
  for (let i = 0; i < layout.length; i++) {
    const item = layout[i];
    if (item !== void 0 && collides(item, layoutItem, threshold)) {
      return item;
    }
  }
  return void 0;
}
function getAllCollisions(layout, layoutItem, threshold = 0) {
  return layout.filter(
    (l) => collides(l, layoutItem, threshold)
  );
}
...
function moveElement(layout, l, x, y, isUserAction, preventCollision, compactType, cols, allowOverlap, collisionThreshold = 0) {
...
function moveElementAwayFromCollision(layout, collidesWith, itemToMove, isUserAction, compactType, cols, collisionThreshold = 0) {
...
//# sourceMappingURL=chunk-B4FK47MR.mjs.map
//# sourceMappingURL=chunk-B4FK47MR.mjs.map