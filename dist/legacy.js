...
function ReactGridLayout(props) {
  ...
  if (!useCSSTransforms) {
    positionStrategy = chunkLUT42EQL_js.absoluteStrategy;
  } else if (transformScale !== 1) {
    positionStrategy = chunkLUT42EQL_js.createScaledStrategy(transformScale);
  } else {
    positionStrategy = chunkLUT42EQL_js.transformStrategy;
  }
  const compactor = chunkLUT42EQL_js.getCompactor(compactType, allowOverlap, preventCollision);
  const constraints = isBounded ? [...chunkLUT42EQL_js.defaultConstraints, chunkLUT42EQL_js.containerBounds] : chunkLUT42EQL_js.defaultConstraints;
  return /* @__PURE__ */ jsxRuntime.jsx(
    chunkQZ7ZMK54_js.GridLayout,
    {
      width,
      gridConfig,
      dragConfig,
...
function ResponsiveReactGridLayout(props) {
  ...
  const compactor = chunkLUT42EQL_js.getCompactor(compactType, allowOverlap, preventCollision);
  return /* @__PURE__ */ jsxRuntime.jsx(
    chunkQZ7ZMK54_js.ResponsiveGridLayout,
    {
      width,
      breakpoint,
      breakpoints,
